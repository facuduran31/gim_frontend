...
export class InscripcionesService {
  ...
  getInscripcionesByIdGimnasio(idGimnasio: number) {
    return this.http.get<Inscripcion[]>(
      `${this.urlApi}/gimnasio/${idGimnasio}`,
      { withCredentials: true },
    );
  }

  getLastInscripcion(idSocio: number) {
    return this.http.get<Inscripcion>(`${this.urlApi}/actual/${idSocio}`, {
      withCredentials: true,
    });
  }

  deleteInscripcion(idInscripcion: number) {
    return this.http.delete(`${this.urlApi}/${idInscripcion}`, {
      withCredentials: true,
    });
  }
  ...
  updateSocioPlan(inscripcion: Inscripcion) {
    ...
    return this.http.put(`${this.urlApi}/${inscripcion.idSocio}`, inscripcion, {
      withCredentials: true,
    });
  }
}
